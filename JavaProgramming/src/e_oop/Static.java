...
public class Static {
	 ...
	 * - static이 붙지 않은 멤버의 명칭: 인스터스 변수, 인스턴스 메서드
	 * 
	 * JVM 메모리구조
	 * - Mathod Area(메서드 영역) : 클래스 멤버가 저장된다. // static이 붙은애가 들어간다
	 * - Heap : 객체가 저장된다. //static이 안 붙은애가 들어간다
	 * - Call Stack(호출 스택) : 현재 호출되어 있는 메서드가 저장되다.
	 */
	...
	public static void main(String[] args) {
		...
		String nextLine = ScanUtil.nextLine();//객체가 아니라 클래스 이름으로 쓴다.
		System.out.println("입력받은 문자열: " + nextLine);
		
		System.out.print("숫자 입력>");
		int nextInt = ScanUtil.nextInt();
		System.out.println("입력받은 문자열: " + nextInt);
